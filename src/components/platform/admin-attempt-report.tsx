...
export function AdminAttemptReport({ attempt, mock, audience = "admin" }: { attempt: Attempt; mock: MockExam; audience?: "admin" | "student" }) {
  ...
  const conceptsExplained = wrong
    .filter((question) => {
      if (conceptsSeen.has(question.subtopic)) return false;
      conceptsSeen.add(question.subtopic);
      return true;
    })
    ...
    <div className="space-y-6 rounded-2xl border border-navy/15 bg-gradient-to-b from-navy/[0.04] to-white p-6 print:border-0 print:bg-white print:p-0 print:shadow-none">
      ...
      {conceptsExplained.length > 0 && (
        <section>
          ...
          <ul className="mt-3 list-disc space-y-1.5 rounded-xl border border-line bg-white p-4 pl-9 text-sm text-navy">
            {conceptsExplained.map((item) => (
              <li key={item.subtopic}><span className="font-bold">{item.subtopic}:</span> {item.explanation}</li>
            ))}
          </ul>
        </section>
      )}
