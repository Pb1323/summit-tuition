...
export function StudentReportView({ attempt, mock }: { attempt: Attempt; mock: MockExam }) {
  ...
  const incorrectQuestions = useMemo(() => questions.filter((question) => !isCorrect(question, attempt.answers[question.id])), [attempt, questions]);
      ...
      <GlowCard className="p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">Use the filters to work through the full paper without losing your place.</p>
          </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2 rounded-2xl bg-cream p-3 print:hidden">
        ...
        <p className="mt-4 text-sm font-bold text-muted print:hidden">{visibleQuestions.length} of {questions.length} questions shown</p>
