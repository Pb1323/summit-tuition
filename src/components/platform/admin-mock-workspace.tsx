...
import { BarChart3, BookOpenText, Clock3, Eye, FilePlus2, PencilLine, Sparkles } from "lucide-react";
...
export function AdminMockWorkspace() {
  ...
  function handleGenerate() {
    ...
    setMessage(`${result.questionCount}/50 questions generated (${result.totalMarks} marks). Review before publishing.`);
  }
      ...
      <GlowCard className="p-6">
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-6">
          ...
          <label>
            ...
            <input type="number" min={10} max={60} value={questionCount} onChange={(event) => setQuestionCount(Number(event.target.value))} className="mt-1 h-11 w-full rounded-xl border border-line px-3 text-sm outline-none focus:border-gold" />
          </label>
          <label>
            <span className="text-xs font-bold uppercase tracking-[0.16em] text-muted">Minutes</span>
            <input type="number" min={10} max={75} value={durationMinutes} onChange={(event) => setDurationMinutes(Number(event.target.value))} className="mt-1 h-11 w-full rounded-xl border border-line px-3 text-sm outline-none focus:border-gold" />
          </label>
        </div>
        <p className="mt-3 text-sm font-semibold text-muted">Full mocks default to 50 questions. Diagnostic and short practice drafts can be generated by lowering the question count manually.</p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <GeneratorMetric icon={<BookOpenText className="h-4 w-4" />} label="Paper profile" value={subject === "English" ? "650-900 word passage" : "30-45% visual reasoning"} />
          <GeneratorMetric icon={<BarChart3 className="h-4 w-4" />} label="Topic mix" value={subject === "English" ? "Retrieval, inference, vocabulary, grammar, cloze" : "Arithmetic, ratio, geometry, data, challenge"} />
          <GeneratorMetric icon={<Clock3 className="h-4 w-4" />} label="Estimate" value={`${questionCount} questions / ${durationMinutes} minutes`} />
        </div>
...
function GeneratorMetric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-line bg-white/80 p-4 shadow-soft">
      <div className="flex items-center gap-2 text-gold-dark">{icon}<span className="text-xs font-black uppercase tracking-[0.16em]">{label}</span></div>
