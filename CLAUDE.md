...
- **2026-07-21 session (share-readiness check)**: before sharing the site more widely, tested the full funnel (welcome → book taster/call or WhatsApp → pricing → pay) end-to-end on both local dev and the live production deploy. Found that `WelcomePricingTabs` (`src/components/sections/welcome-pricing-tabs.tsx`, used only on `/welcome`) rendered a plain `Button href={tier.ctaHref}` for its Pro/Max CTAs instead of `CheckoutButton` — the only place on the site where the Stripe-checkout-or-WhatsApp-fallback pattern (see the 2026-07-21 WhatsApp entry below) wasn't wired in, so paying visitors on the WhatsApp-shared page landed on a plain enquiry form instead of the WhatsApp payment flow. Fixed to match `pricing-card.tsx`'s pattern. Also confirmed (don't re-litigate without a reason): `RESEND_API_KEY`/`EMAIL_FROM`/`ADMIN_NOTIFICATION_EMAIL`/`NEXT_PUBLIC_CALENDLY_URL` are all set in Vercel Production, so `BookACallPage`'s enquiry form and calendar are live, not showing dev placeholders.
