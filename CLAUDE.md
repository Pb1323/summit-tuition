...
- Stripe: Pro/Max checkout is fully wired and tested end-to-end against test-mode Prices (see Recent Feature State, 2026-07-20 Stripe session) — but only locally. Production (Vercel) has no Stripe env vars set at all yet, deliberately, so `/api/checkout` there still shows the "coming soon" fallback. `NEXT_PUBLIC_SITE_URL` is also not yet set in Vercel, which would send Checkout's success/cancel redirects to the wrong domain once Stripe is enabled there — set it before going further. Real (live-mode) payments are blocked on the founder buying a domain and completing Stripe business/bank activation (in progress as of 2026-07-20); see `status.md` for the exact staged rollout order.
